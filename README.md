...
The "things" in a computer exist only as *representations* -- in other words, it's all just structured information. Traditionally, everything is stored as raw data without any explicit structure, and software tools must transform it into a structural form that can be viewed or manipulated in some meaningful way. If instead, everything was stored in an explicit structural form *to begin with*, then you could modify it into *any* form, rather than being only to what specific software tools allow you to do. Thus, everything will be composed of structural building blocks that can be assembled into any ad-hoc form, and create any representation you like. (I've chosen [JSON](http://www.json.org/) for its support of ad-hoc "objects").
