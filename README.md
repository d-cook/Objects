...
The "things" in a computer exist only as *representations* -- in other words, it's all just structured information. Traditionally, everything is stored as raw data without any explicit structure, and software tools must transform it into a structural form that can be viewed or manipulated in some meaningful way. If instead that structure was explicit *outside* of the software tools, then it could all be manipulated directly in place, rather than having to rely on software tools to create the structure, and only being limited to the specific structures and manipulations that each tool allows. (I've chosen [JSON](http://www.json.org/) for its support of ad-hoc "objects").
