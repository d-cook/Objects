...
All that matters here is that there is *any* interface to begin with, because the end-user can then manipulate it into whatever they want. This calls for a minimal interface just good enough to allow one to see and manipulate the structure of everything. And by making this a *visual* interface (GUI), the "everything" that the user can see & manipulate also includes *the very interface used to do so*. So in addition to being able to modify the structure and behavior of all things, this also opens the door for creating whatever *graphical* representation or tool that is most ideal for each scenario. For example, editing the size & position of a ball by *clicking and dragging A BALL*, rather than by *editing NUMERIC DATA about a ball*. (Try *that* on as a replacement for [macros](https://en.wikipedia.org/wiki/Macro_(computer_science)) & [DSL](https://en.wikipedia.org/wiki/Domain-specific_language)s!) (see Brett Victor's "[Dynamic Visualizations](http://worrydream.com/DrawingDynamicVisualizationsTalk)" and "[Learnable Programming](http://worrydream.com/LearnableProgramming/)").
