...
All that matters here is that there is *any* interface to begin with, because the end-user can then manipulate it into whatever they want. Thus, the goal is a minimal interface just good enough to allow one to see and manipulate the structure of everything. Implementing this as a *graphical* interface (GUI) will allow the user to also customize all graphical behaviors as well, since the code for rendering graphics and responding to user-interactions will itself be exposed to the end-user for manipulation as desired.
