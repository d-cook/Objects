...
The idea is that it can be manipulated into the most ideal interface/tool for the task at hand. For example, editing the size & position of a ball by *clicking and dragging A BALL*, rather than by *editing NUMERIC DATA about a ball*. (Try *that* on as a replacement for [macros](https://en.wikipedia.org/wiki/Macro_(computer_science)) & [DSL](https://en.wikipedia.org/wiki/Domain-specific_language)s!) (see Brett Victor's "[Dynamic Visualizations](http://worrydream.com/DrawingDynamicVisualizationsTalk)" and "[Learnable Programming](http://worrydream.com/LearnableProgramming/)").
