...
All that matters here is that there is *any* interface to begin with, because the end-user can then manipulate it into whatever they want. Thus, the goal is a minimal interface just good enough to allow one to see and manipulate the structure of everything. Implementing this as a *graphical* interface (GUI) will also allow the user to also customize all graphical behaviors, since the code for rendering graphics and responding to user-interaction will be exposed (just like everything else) through this interface for end-user manipulation.

Thus, the user-interface will start out as a visualization of generic data-trees, but can be further modified to create the ideal interface for the task at hand. For example, editing the size & position of a ball by clicking and dragging *a ball*, rather than by editing numeric data *about* a ball. (Try *that* on as a replacement for [macros](https://en.wikipedia.org/wiki/Macro_(computer_science)) & [DSL](https://en.wikipedia.org/wiki/Domain-specific_language)s!) (Brett Victor demonstrates concept well: "[Dynamic Visualizations](http://worrydream.com/DrawingDynamicVisualizationsTalk)", "[Learnable Programming](http://worrydream.com/LearnableProgramming/)", "[Magic Ink](http://worrydream.com/MagicInk)")
